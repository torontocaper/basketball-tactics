...
var layers: Array[CourtLayer]

@onready var floor_layer: CourtLayer = $FloorLayer
@onready var territory_layer: CourtLayer = $TerritoryLayer
@onready var points_layer: CourtLayer = $PointsLayer
@onready var grid_layer: CourtLayer = $GridLayer

func _ready() -> void:
	layers = [floor_layer, territory_layer, points_layer, grid_layer]
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_pressed() and event is InputEventMouseButton:
		...
		var clicked_tile_coords = floor_layer.local_to_map(click_local_position)
