...
var players_on_court: Array[Player]:
	set(value):
		...
		for player in players_on_court:
			player.court_map = court_map
		court_map.set_occupied_cells(players_on_court)
...
func _ready() -> void:
	print_debug("Court ready at %s" % Time.get_ticks_msec())
	_connect_signals()
...
func _on_input_event(_viewport:Node, event:InputEvent, _shape_idx: int):
	if event.is_pressed() and event is InputEventMouseButton:
		...
		print_debug("Court clicked!\nLocal mouse: %s\nTile index: %s" % [get_local_mouse_position(), clicked_tile_coords])
